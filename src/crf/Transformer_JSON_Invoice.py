...
"""
    Node and edge feature transformers to extract features for JSON Invoice
...
class NodeTransformerInvoice(Transformer):
    """
    Characterising the invoice field

    "features": {
      "LineMathFeatures.isFactor": "NO", 
      "LineMathFeatures.isProduct": "NO", 
      "RawText": "XX0", 
      "TextPatterns": "XX0", 
      "bottomMargin": 0.4228028357028961, 
      "bottomMarginRelative": 0.011084718629717827, 
      "hasDigits": "YES", 
      ...
      "isKnownZip": "NO", 
      "leftAlignment": 35.0, 
      "leftMargin": 0.1132287010550499, 
      "leftMarginRelative": 0.006726457271724939, 
      "length": 3.0, 
      "lineSize": 8.0, 
      "lineWhiteSpace": 0.9724962711334229, 
      "pageHeight": 1263.0, 
      "pageWidth": 892.0, 
      "parsesAsAmount": "YES", 
      "parsesAsDate": "NO", 
      "parsesAsNumber": "NO", 
      "positionOnLine": 0.125, 
      "rightMargin": 0.13228699564933777, 
      "rightMarginRelative": 0.006726457271724939, 
      "topMargin": 0.41013461351394653, 
      "topMarginRelative": -0.020585905760526657, 
      "verticalPosition": 0.6499999761581421
    },
    """
    nbYesNo = len(["LineMathFeatures.isFactor", "LineMathFeatures.isProduct", "hasDigits"
                    , "isKnownCity", "isKnownCountry", "isKnownZip"
                    , "parsesAsAmount", "parsesAsDate", "parsesAsNumber"])
    def transform(self, lNode):
#         a = np.empty( ( len(lNode), 5 ) , dtype=np.float64)
#         for i, blk in enumerate(lNode): a[i, :] = [blk.x1, blk.y2, blk.x2-blk.x1, blk.y2-blk.y1, blk.fontsize]        #--- 2 3 4 5 6 
        a = np.empty( (len(lNode), self.nbYesNo), dtype=np.float64)
        for i, blk in enumerate(lNode): 
            a[i,0:self.nbYesNo] = blk.lfYesNoFeat
        return a
